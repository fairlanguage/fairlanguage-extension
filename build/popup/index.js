...
var _default = function _default(msg) {
  if (__DEV__) {
    console.error("%c * Fairlanguage - ".concat(msg), styles);
  } else {
...
function () {
  ...
  _createClass(StorageController, null, [{
    ...
    value: function getSettings() {
      return new Promise(function (resolve, reject) {
        chrome.storage.local.get(['settings'], function (storage) {
          (0, _helperLogger.default)('Reading general settings from local storage...');

          if (storage.settings) {
            (0, _helperLogger.default)('Found general settings:');
            (0, _helperLogger.default)(JSON.stringify(storage.settings));
            ...
            chrome.storage.local.set({
              ...
              (0, _helperLogger.default)('Wrote initial settings to local storage:');
    ...
    value: function getHostSettings() {
      ...
      return new Promise(function (resolve, reject) {
        ...
        function () {
          ...
          /*#__PURE__*/
          regeneratorRuntime.mark(function _callee(storage) {
                ...
                switch (_context.prev = _context.next) {
                  case 0:
                    ...
                    return chrome.storage.local.set({
                      ...
                      (0, _helperLogger.default)('Wrote current host settings to local storage:');
                      (0, _helperLogger.default)(JSON.stringify(settings));
    ...
    value: function setEnabled() {
      ...
      return new Promise(function (resolve, reject) {
        chrome.storage.local.get(['settings'], function (storage) {
          (0, _helperLogger.default)('Reading current settings from local storage...');
          var settings = storage.settings;
          (0, _helperLogger.default)(JSON.stringify(settings));
          settings.enabled = mode === 'toggle' ? !settings.enabled : mode;
          (0, _helperLogger.default)('Writing new settings to local storage:');
          (0, _helperLogger.default)(JSON.stringify(settings));
          chrome.storage.local.set({
            settings: settings
          }, function () {
            if (settings) {
              (0, _helperLogger.default)('Wrote new settings to local storage:');
              (0, _helperLogger.default)(JSON.stringify(settings));
              resolve(settings);
            } else {
...
var STRING_STATUS_DISABLED = 'Die Extension ist aktiviert und kann genutzt werden.<br/>Hiermit ist unserer Datennutzung zugestimmt. Wenn du damit nicht mehr einverstanden bist kannst du die Extension jederzeit deaktivieren.';
var STRING_STATUS_ENABLED = "\nUm dir Feedback und Empfehlungen zu deiner Sprache geben zu k\xF6nnen, \nm\xFCssen wir diese an unseren Server senden und dort analysieren.\nWeder speichern wir deine Daten noch geben wir sie weiter und werden dies\nauch niemals tun.\nMit der Nutzung dieser Extension stimmst du der Verarbeitung deiner \nDaten und unserer Datenschutzerkl\xE4rung ausdr\xFCcklich zu.";
var STRING_BUTTON_ENABLE = 'Ja, damit bin ich einverstanden.';
var STRING_BUTTON_DISABLE = 'Extension deaktivieren';
var STRING_ENABLE_HOST = 'Auf dieser Seite aktivieren';
var STRING_DISABLE_HOST = 'Auf dieser Seite deaktivieren';
var STRING_READ_MORE = 'Mehr zu Fairlanguage';
var STRING_SUPPORTED = "\nDiese Website wird bereits unterst&uuml;tzt. \nSollte dennoch ein Fehler auftreten schreibe uns bitte eine Mail.";
var STRING_EXPERIMENTAL = "\nEs kann auf dieser Website noch zu Fehlern kommen. Daher haben haben wir die Extension \nauf dieser Seite zun&auml;chst deaktiviert. Du kannst sie jedoch im experimentellen Modus aktivieren.\n";
/**
 * Elements
 */
...
var viewEnabled = document.getElementById('view-enabled');
var statusHost = document.getElementById('status-host');
var displaySupported = document.getElementById('status-supported');
var statusSupportedString = document.getElementById('status-supported-string'); // General settings
...
var buttonEnabled = document.getElementById('button-enabled'); // Host settings

var captionHost = document.getElementById('caption-host');
var stringHost = document.getElementById('string-host');
var captionHostEnabled = document.getElementById('caption-host');
var displayHost = document.getElementById('active-status');
var buttonHostEnable = document.getElementById('active-button-enable'); // Dev/Hosts

var captionDev = document.getElementById('caption-dev');
var statusHosts = document.getElementById('status-hosts');
var buttonResetHosts = document.getElementById('button-reset-hosts'); // Version
...
heading.style.display = _config.default.options.heading ? 'flex' : 'none';
subHeading.style.display = _config.default.options.subHeading ? 'flex' : 'none';
/**
 * Status host
 */
...
captionSettings.style.display = _config.default.options.settingsString ? 'flex' : 'none'; // General settings string

displaySettings.style.display = _config.default.options.settingsString ? 'flex' : 'none'; // General settings caption

captionEnabled.style.display = _config.default.options.settingsCaption ? 'flex' : 'none'; // General settings status

statusEnabled.style.display = _config.default.options.settings ? 'flex' : 'none'; // General settings button

buttonEnabled.style.display = _config.default.options.settings ? 'flex' : 'none';
...
captionHost.style.display = _config.default.options.hostSettingsCaption ? 'flex' : 'none'; // Host settings string

stringHost.style.display = _config.default.options.hostSettingsString ? 'flex' : 'none'; // Host settings caption

displayHost.style.display = _config.default.options.hostSettingsDisplay ? 'flex' : 'none'; // Host settings status

captionHostEnabled.style.display = _config.default.options.hostSettingsCaption ? 'flex' : 'none'; // Host settings button

buttonHostEnable.style.display = _config.default.options.buttonHostEnable ? 'flex' : 'none';
buttonHostEnable.addEventListener('click', function () {
  chrome.tabs.query({
    ...
    chrome.tabs.sendMessage(tabs[0].id, {
      command: buttonHostEnable.textContent === STRING_DISABLE_HOST ? 'host-disable' : 'host-enable'
...
displayVersion.style.display = _config.default.options.displayVersion ? 'flex' : 'none';
...
chrome.runtime.onMessage.addListener(function (settings) {
  ...
  buttonHostEnable.textContent = settings.host.enabled ? STRING_DISABLE_HOST : STRING_ENABLE_HOST; // Reload current tab
...
chrome.runtime.onMessage.addListener(function (data) {
  ...
  viewEnabled.style.display = settings.enabled || settings.enabled === undefined ? 'flex' : 'none'; // Reload current tab
...
var getCurrentHostSupportFromHosts = function getCurrentHostSupportFromHosts(currentHost) {
  var currentHostSupport = 'experimental';
  var _iteratorNormalCompletion = true;
  var _didIteratorError = false;
  var _iteratorError = undefined;

  try {
    for (var _iterator = _hosts.default[Symbol.iterator](), _step; !(_iteratorNormalCompletion = (_step = _iterator.next()).done); _iteratorNormalCompletion = true) {
...
var getCurrentHostSettings = function getCurrentHostSettings() {
  chrome.tabs.query({
    ...
    _storage.default.getHostSettings(currentHost).then(function (settings) {
      stringHost.value = JSON.stringify(settings);
      displayHost.textContent = settings.enabled === null ? 'not set' : settings.enabled;
      var currentHostSupport = getCurrentHostSupportFromHosts(currentHost);
      buttonHostEnable.textContent = settings.enabled === true || settings.enabled === null ? STRING_DISABLE_HOST : STRING_ENABLE_HOST; // buttonHostEnable.style.color = settings.enabled === true || settings.enabled === null ? 'grey' : 'black';
      ...
      displaySupported.textContent = "".concat(currentHost);
